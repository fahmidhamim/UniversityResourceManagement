<?php
include_once('../../service/mysqlcon.php');

if(isset($_GET['number'])){
    $room_number = $_GET['number'];
    
    // Delete past bookings automatically
    $current_date = date('Y-m-d');
    $delete_sql = "DELETE FROM bookings WHERE booking_date < '$current_date'";
    mysqli_query($link, $delete_sql);

    // Check for active bookings (today or future)
    $sql = "SELECT * FROM bookings WHERE room_number='$room_number' AND booking_date >= '$current_date'";
    $res = mysqli_query($link, $sql);

    if(mysqli_num_rows($res) > 0) {
        echo "<p style='color: red;'>Not available</p>";
        echo "<a href='viewbooking.php?number=$room_number'><button>View</button></a>";
    } else {
        echo "<p style='color: green;'>The room is available.</p>";
    }
} else {
    echo "Room number not provided.";
}
?>
